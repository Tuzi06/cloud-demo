...
def grabing(driver,link,user,idx,photos,detail_text):
    ...
    postInfo['user-follow'] = user['follow']
    ...
    if len(driver.find_elements(By.CLASS_NAME,'swiper-wrapper')) == 0:
        postInfo['video'] = True
        wait_for_page(driver,'xgplayer-poster')
        postInfo['video'] = True
        container = driver.find_element(By.CLASS_NAME,'media-container')
        poster = container.find_element(By.CLASS_NAME,'xgplayer-poster')
        poster = poster.get_attribute('style')
        poster = poster.split('"')[1]
        post['picture'] = {'%s-%i.png'%(user['user-id'],idx):poster}
        idx += 1
    else:
